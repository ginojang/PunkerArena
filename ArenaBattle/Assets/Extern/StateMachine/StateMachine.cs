...
namespace MonsterLove.StateMachine
{
    ...
    public class StateMachine<T> : IStateMachine where T : struct, IConvertible, IComparable
    {
        ...
        {
            var kList = new List<object>(stateLookup.Keys);
            for (int i = 0; i < kList.Count; i++)
            {
                if (kList[i].ToString() == State.ToString())
                {
                    if (i + 1 >= kList.Count)
                        return false;

                    ChangeState((T)kList[i + 1]);
                    break;
                }
            }
            //stateLookup
        ...
        public bool ChangeSubNextState(string[] nextState)
        {
            var kList = new List<object>(stateLookup.Keys);
            for (int i = 0; i < kList.Count; i++)
            {
                if (kList[i].ToString() == nextState[0])
                {
                    if (i + 1 >= kList.Count)
                        return false;
        ...
        public static void Destroy(MonoBehaviour component)
        {
            component.GetComponent<StateMachineRunner>().Destroy();
            //component.GetComponent<StateMachineRunner>().Destroy();
