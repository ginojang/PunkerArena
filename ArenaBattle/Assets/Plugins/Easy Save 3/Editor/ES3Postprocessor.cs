...
public class ES3Postprocessor : UnityEditor.AssetModificationProcessor
{
	...
	static void Update()
	{
		...
		{
			while(changedSinceLastSave.Count > 0)
			{
				if(Time.realtimeSinceStartup - timeStarted > 0.02f)
					return;
				refMgr.AddDependencies(new UnityEngine.Object[]{changedSinceLastSave.Dequeue()});
			}
		}
	}
	...
	{
		if(defaultSettings.autoUpdateReferences && refMgr != null)
			refMgr.GeneratePrefabReferences();
	...
	private static UndoPropertyModification[] OnPostProcessModifications(UndoPropertyModification[] propertyModifications)
	{
		...
		foreach (UndoPropertyModification m in propertyModifications)
		{
			...
			// Only update if Auto Update References is enabled
			if(!defaultSettings.autoUpdateReferences || refMgr == null)
				return propertyModifications;
	
			// If this is a scene object, and it's not a reference manager or manager object, add it.
			if(!AssetDatabase.Contains(obj) && obj != refMgr && obj != refMgr.gameObject)
				changedSinceLastSave.Enqueue(obj);
			else if(ES3EditorUtility.IsPrefabInAssets(obj) && obj is Component)
				refMgr.GeneratePrefabReferences(((Component)obj).gameObject);
		}
		return propertyModifications;
	}
