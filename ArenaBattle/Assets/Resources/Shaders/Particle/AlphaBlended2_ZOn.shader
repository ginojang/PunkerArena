...
Shader "VESTINEL/Particles/Alpha Blended (2) ZOn" 
{
	Properties
	{
	...
	Subshader 
	{
		...
		Pass
        {
			...
			struct v2f
			{
				...
				#ifdef DISTORTION_ON
					half4 distortion_uv_ff	: TEXCOORD1;
				#else
					half1 distortion_uv_ff	: TEXCOORD1;
				#endif
			};
			...
			v2f vert(VS_IN v)
			{
				...
				#ifdef SECOND_UV_ON
					o.uv.zw = TRANSFORM_TEX (v.texcoord1, _SubTex);  
				#else
				...
				#ifdef DISTORTION_ON
					o.distortion_uv_ff.yz = TRANSFORM_TEX (v.texcoord, _DistortionTex);  
				#endif

				o.distortion_uv_ff.x = GetFogFactor(o.pos, unity_FogStart, unity_FogEnd);
				return o;
			}
			...
			{
				#ifdef DISTORTION_ON
					half2 distortionOffset = tex2D(_DistortionTex, i.distortion_uv_ff.yz).xy * 2-1;
					distortionOffset *= _DistortionRate;

					i.uv += distortionOffset.xyxy;
				#endif
